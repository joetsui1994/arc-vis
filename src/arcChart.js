...
function ArcChart(props) {
    ...
    const highlightPath = (name) => {
        if (name === null) {
            d3.selectAll(".masks").each(function(d) {
                d3.select(`#${d.data.name}-mask`)
                    .transition(d.data.name)
                    .duration(transitionDuation)
                    .attr("fill-opacity", 0);
            });
        } else {
            const highlightedNode = d3.select(`#${name}`).datum();
            const sequence = highlightedNode.ancestors().reverse().slice(1);
            ...
            d3.selectAll(".masks")
                ...
                .each(function(d) {
                    d3.select(`#${d.data.name}-mask`)
                        .transition(d.data.name)
                        .duration(transitionDuation)
                        .attr("fill-opacity", 1 - unhoveredOpacity);
                });
    ...
    const calcDivAngle = (d, start = true) => {
        ...
        const preDivsSum = d.data.divs
            .slice(0, divId)
            .reduce((sum, div) => sum + div.num, 0);
        const totalAngle = d.x1 - d.x0;
        const newAngle =
            d.x0 +
            (preDivsSum / d.data.num) * totalAngle +
            (start ? 0 : (d.div.num / d.data.num) * totalAngle);
        return newAngle;
    };
    ...
    const drawChart = () => {
        ...
        const addInnerRadius =
            ((innerRadius - 1) * (width / 2 - chartPadding)) / treeHeight; // magnitute of addInnerRadius must not exceed the width of a single arc
        ...
        svg
            ...
            .style(
                ...
                1 -
                (d.depth - 1) *
                Math.min(maxOpacityGradient, (1 - minOpacity) / treeHeight)
            )
